...
const ValidatorRow = (props) => {
    ...
    return <Card body>
        <SentryBoundary><Row className="validator-info">
            ...
            {(!props.inactive)?<Col className="commission data" xs={{size:4}} md={{size:1,offset:0}} lg={2}><i className="material-icons d-sm-none">call_split</i> <span>{(props.validator.commission&&props.validator.commission.commission_rates)?numbro(props.validator.commission.commission_rates.rate).format('0.00%'):'N/A'}</span></Col>:''}
            {(!props.inactive)?<Col className="uptime data" xs={{size:2,order:"last"}} md={4}><span>--</span></Col>:''}
